...
namespace AStar
{
    class Pathfinder
    {
        ...
        public Pathfinder(Node start, Node end, Grid grid)
        {
            ...
            start.ValG = Distance(start.PositionX, start.PositionY, end.PositionX, end.PositionY);
            ...
            while (openList != null)
            {
                ...
                {
                    current.Cost = current.ValG + Distance(current.PositionX, current.PositionY, successor.PositionX, successor.PositionY);
                    if (openList.Contains(successor))
                    {
                        if (successor.ValG <= current.Cost)
                        {
                            continue;
                        }
                    }
